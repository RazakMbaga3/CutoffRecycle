  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {locations.map((location, index) => (
              <div key={index} className="bg-white rounded-xl p-6 shadow-md" onClick={()=>handleMarkerClick(location)}>
  ...
  fullscreenControl: false}}
      >
        {mapLoaded &&
          ...
          {locations.map(
          (loc) =>
            hoveredMarker === loc.city && (
              <InfoWindow
                key={`infowindow-${loc.city}`}
                position={loc.coordinates}
                options={{ pixelOffset: new window.google.maps.Size(0, -30) }}
              >
                <div>
                  <h2 className="font-bold">{loc.city}</h2>
                  <p>This is {loc.city} city marker.</p>
                </div>
              </InfoWindow>
            )
        )}
